...
// confusing here, but the static member of the class template needs to define the static member per T.
template <class T>
size_t Array<T>::s_defaultSize = 10;

template <class T>
// default ctor allocates defaultSize of elements  
Array<T>::Array() : m_size(s_defaultSize) { // NOW: use dfault size instead of only 10
	m_data = new T[s_defaultSize];
}
...
template<class T>
size_t Array<T>::DefaultSize(){
	return s_defaultSize;
}


template<class T>
void Array<T>::DefaultSize(size_t size){
	s_defaultSize = size;
}
